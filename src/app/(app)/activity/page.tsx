...
function activityIndicator(type: string, title: string) {
  ...
  // Status indicators
  if (titleLower.includes("active") || titleLower.includes("up")) return { color: "bg-emerald-500", label: "UP" as const };
  if (titleLower.includes("down")) return { color: "bg-red-500", label: "DOWN" as const };

  // Inventory/Asset specific
  ...
  // NEW: Asset Instance / Serial tracking indicators
  ...
  if (t.includes("fault") || titleLower.includes("faulty")) return { color: "bg-orange-500", label: "FAULT" as const };
  ...
  if (t.includes("updated") || t.includes("changed")) return { color: "bg-blue-500", label: "UPDATED" as const };
...
function entityHref(entityType: string | null, entityId: string | null) {
  ...
  // NEW: Handle deep unit links if needed in future
...
// Updated Options to include Asset Instance Tracking
const actionOptions = [
...
export default async function ActivityPage({
  ...
  const title = q || type || period !== "ALL"
    ? `Recent Activity${q ? ` — Search: ${q}` : ""}${type ? ` — Action: ${getActionLabel(type)}` : ""}${period !== "ALL" ? ` — Period: ${period}` : ""}`
    : "Recent Activity";
