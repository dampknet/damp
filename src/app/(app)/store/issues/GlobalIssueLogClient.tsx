...
type IssueRow = {
  ...
  inventoryItem: {
    ...
    entityCodePreview: string;    // ✅ replaces serialNumber
  };
  inventorySite: {
    id:   string;
    name: string;
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className={dark ? "mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-400" : "mt-3 max-w-2xl text-sm font-medium leading-6 text-[#857f76]"}>
                Review issued items and equipment movement across all warehouse sites.
              </p>
              <div className={dark
          ...
          <div className={dark
            ...
            <form className="grid gap-3 md:grid-cols-4">
              <input name="q" defaultValue={q}
                placeholder="Search requester, site, item, item code, entity code..."
                className={dark
              ...
              <div className="flex items-center gap-2 md:col-span-4">
                ...
                {(q || status !== "ALL" || type !== "ALL") && (
                  <Link href="/store/issues" className={dark
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <SummaryCard dark={dark} label="Issued"    value={String(summary.issuedCount)}
            sub="currently open"      accent={dark ? "bg-[linear-gradient(90deg,#f59e0b,#fbbf24)]" : "bg-[#b08b2c]"} />
          <SummaryCard dark={dark} label="Returned"  value={String(summary.returnedCount)}
            sub="closed records"      accent={dark ? "bg-[linear-gradient(90deg,#10b981,#34d399)]" : "bg-[#2a7d52]"} />
          ...
          <SummaryCard dark={dark} label="Other"     value={String(summary.othersCount)}
            sub="accessories etc"     accent={dark ? "bg-[linear-gradient(90deg,#f97316,#fb923c)]" : "bg-[#c8611a]"} />
        </div>
                  ...
                  >
                    ...
                    <td className={dark ? "px-5 py-3 text-slate-100" : "px-5 py-3 text-[#1a1814]"}>
                      ...
                      <div className="mt-0.5 flex flex-col gap-0.5">
                        {row.inventoryItem.itemCode && (
                          <span className={`font-mono text-[10px] ${dark ? "text-slate-500" : "text-[#8b857c]"}`}>
                            {row.inventoryItem.itemCode}
                          </span>
                        )}
                        {row.inventoryItem.entityCodePreview !== "-" && (
                          <span className="font-mono text-[10px] italic text-sky-500">
                            {row.inventoryItem.entityCodePreview}
                          </span>
                        )}
                      </div>
                    ...
                    <td className={dark ? "px-5 py-3 text-slate-300" : "px-5 py-3 text-[#5d584f]"}>
                      {row.quantity}{row.inventoryItem.unit ? ` ${row.inventoryItem.unit}` : ""}
                    </td>
                    <td className={dark ? "px-5 py-3 text-slate-300" : "px-5 py-3 text-[#5d584f]"}>
