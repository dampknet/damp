...
export default function CheckedOutEquipmentClient({
  role, q, items, exportRows, exportCols,
}: {
  role:       string;
  q:          string;
  items:      Row[];
          ...
          <div className={dark
            ...
            <form className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <input name="q" defaultValue={q}
                placeholder="Search site, item, item code, entity code, requester..."
              ...
              {q && (
                <Link href="/store/checkouts" className={dark
              ...
              <tbody className={dark ? "divide-y divide-white/8" : "divide-y divide-[#eee7dd]"}>
                {items.length === 0 ? (
                  ...
                  <tr key={row.id} className={dark ? "hover:bg-white/5" : "hover:bg-[#fcfaf7] transition-colors"}>
                    <td className={dark ? "px-5 py-4 text-slate-300" : "px-5 py-4 text-[#5d584f]"}>
                      {index + 1}
                    </td>

                    <td className="px-5 py-4">
                      <Link href={`/store/sites/${row.inventorySite.id}/issues`}
                        className={dark
                          ? "font-bold text-slate-100 hover:underline"
                          : "font-bold text-[#1a1814] hover:underline"
                        }
                      >
                    ...
                    <td className="px-5 py-4">
                      <div className={dark ? "font-bold text-slate-100" : "font-bold text-[#1a1814]"}>
                      ...
                      <div className="mt-1 flex flex-col gap-0.5">
                        ...
                        {row.inventoryItem.entityCodePreview !== "N/A" && (
                    ...
                    <td className="px-5 py-4">
                      <div className={dark ? "font-semibold text-slate-300" : "font-semibold text-[#5d584f]"}>
                        {row.requesterName}
                      </div>
