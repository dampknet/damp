...
export default function ReturnEquipmentClient({
  ...
  issue: {
    ...
    conditionAtIssue:  "NEW" | "UNUSED" | "USED" | "FAULTY" | null;
    item: {
      id:                string;
      name:              string;
      ...
      unit:              string | null;
      status:            string;
      condition:         "NEW" | "UNUSED" | "USED" | "FAULTY" | null;
    };
  };
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 md:px-6">
        <section className={dark
          ...
          <p className={dark ? "mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-400" : "mt-3 max-w-2xl text-sm font-medium leading-6 text-[#857f76]"}>
            Complete the return record, update the item condition, and close this issue.
          </p>
          ...
          <div className={dark
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              <Info dark={dark} label="Item Code"      value={issue.item.itemCode ?? "—"} />
              <Info dark={dark} label="Entity Codes"   value={issue.item.entityCodePreview} />
              ...
              <Info dark={dark} label="Expected Return"
                value={issue.expectedReturnDate ? new Date(issue.expectedReturnDate).toLocaleString() : "—"} />
              <Info dark={dark} label="Condition At Issue" value={issue.conditionAtIssue ?? "—"} />
              <Info dark={dark} label="Current Condition"  value={issue.item.condition ?? "—"} />
            </div>
            <div className="mt-4">
            ...
            <section className={dark
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <Field label="Returned By" dark={dark}>
                  <input name="returnedBy" placeholder="Full name"
                    required title="Returned By" className={inputCls} />
                </Field>
                <Field label="Return Contact" dark={dark}>
                  <input name="returnContact" placeholder="Phone number or contact"
                    required title="Return Contact" className={inputCls} />
                </Field>
              </div>

              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <Field label="Returned At" dark={dark}>
                  <input name="returnedAt" type="datetime-local" defaultValue={nowValue}
                    required title="Returned At"
                    className={dark
                ...
                <Field label="Return Condition" dark={dark}>
                  <select name="returnCondition"
                    defaultValue={issue.item.condition ?? "USED"}
              ...
              <div className="mt-4">
                <Field label="Return Note (optional)" dark={dark}>
                  <textarea name="returnNote" rows={4}
                    placeholder="Remarks about returned condition or observations..."
                    title="Return Note" className={inputCls} />
                </Field>
              </div>
            </section>
...
function Info({ label, value, dark }: { label: string; value: string; dark: boolean }) {
  return (
    <div>
      ...
      <div className={dark ? "mt-1 text-sm text-slate-200" : "mt-1 text-sm text-[#1a1814]"}>
        {value}
      </div>
    </div>
  );
