...
export default function IssueLogClient({
  role, canEdit, site, q, status, type,
  summary, issues, exportRows, exportCols,
}: {
  role:     string;
  canEdit:  boolean;
    ...
    <div className={dark
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
        ...
        <section className={dark
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className={dark ? "mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-400" : "mt-3 max-w-2xl text-sm font-medium leading-6 text-[#857f76]"}>
                Review all issued items, checked-out equipment, requester info, and return status.
              </p>
              <div className={dark
          ...
          <div className={dark
            ...
            <form className="grid gap-3 md:grid-cols-4">
              <input name="q" defaultValue={q}
                placeholder="Search requester, item, purpose, entity code..."
                title="Search issues"
                className={dark
              ...
              >
                ...
                <option value="TOOLS_AND_PARTS">Tools & Parts</option>
                <option value="GENERAL">General</option>
                <option value="COOLING_INFRASTRUCTURE">Cooling</option>
                <option value="CABLES_AND_ELECTRONICS">Cables & Electronics</option>
              </select>
              ...
              <div className="flex items-center gap-2 md:col-span-4">
                ...
                {(q || status !== "ALL" || type !== "ALL") && (
                  <Link href={`/store/sites/${site.id}/issues`} className={dark
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <SummaryCard dark={dark} label="Issued"    value={String(summary.issuedCount)}
            sub="currently out" accent={dark ? "bg-[linear-gradient(90deg,#f59e0b,#fbbf24)]" : "bg-[#b08b2c]"} />
          <SummaryCard dark={dark} label="Returned"  value={String(summary.returnedCount)}
            sub="brought back"  accent={dark ? "bg-[linear-gradient(90deg,#10b981,#34d399)]" : "bg-[#2a7d52]"} />
          <SummaryCard dark={dark} label="Equipment" value={String(summary.equipmentCount)}
            sub="equipment issues" accent={dark ? "bg-[linear-gradient(90deg,#3b82f6,#60a5fa)]" : "bg-[#1d5fa8]"} />
          <SummaryCard dark={dark} label="Other Items" value={String(summary.othersCount)}
            sub="accessories, tools etc" accent={dark ? "bg-[linear-gradient(90deg,#f97316,#fb923c)]" : "bg-[#c8611a]"} />
        </div>
        ...
        <section className={dark
          ...
          <div className="print-only px-5 py-4">
            <div className="text-lg font-semibold text-gray-900">{printTitle}</div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className={dark ? "divide-y divide-white/8" : "divide-y divide-[#eee7dd]"}>
                {issues.length === 0 ? (
                  <tr>
                    <td colSpan={11} className={dark ? "px-5 py-10 text-center text-slate-500" : "px-5 py-10 text-center text-gray-600"}>
                      No issue records found.
                    </td>
                  </tr>
                ) : issues.map((issue, index) => (
                  <tr key={issue.id} className={dark ? "hover:bg-white/5" : "hover:bg-[#fcfaf7]"}>
                    ...
                    <td className={dark ? "px-5 py-3 text-slate-100" : "px-5 py-3 text-[#1a1814]"}>
                      ...
                      <div className="mt-0.5 flex flex-col gap-0.5">
                        {issue.inventoryItem.itemCode && (
                          <span className={`font-mono text-xs ${dark ? "text-slate-500" : "text-[#8b857c]"}`}>
                            {issue.inventoryItem.itemCode}
                          </span>
                        )}
                        {issue.inventoryItem.entityCodePreview !== "-" && (
                          <span className="font-mono text-[10px] text-sky-500 italic">
                            {issue.inventoryItem.entityCodePreview}
                          </span>
                        )}
                      </div>
                    ...
                    <td className={dark ? "px-5 py-3 text-slate-300" : "px-5 py-3 text-[#5d584f]"}>
                      {ITEM_TYPE_LABEL[issue.itemType] ?? issue.itemType}
                    </td>

                    <td className={dark ? "px-5 py-3 text-slate-300" : "px-5 py-3 text-[#5d584f]"}>
                      {issue.quantity}{issue.inventoryItem.unit ? ` ${issue.inventoryItem.unit}` : ""}
                    </td>
                    ...
                    <td className={dark ? "px-5 py-3 text-slate-300" : "px-5 py-3 text-[#5d584f]"}>
                      {issue.expectedReturnDate ? new Date(issue.expectedReturnDate).toLocaleString() : "-"}
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <span className={badgeStatus(issue.status, dark)}>{issue.status}</span>
                    </td>

                    <td className="px-5 py-3 no-print">
                      {issue.itemType === "EQUIPMENT" && issue.status === "ISSUED" ? (
                        <Link href={`/store/sites/${site.id}/issues/${issue.id}/return`}
                          className={dark
                        ...
                        <span className={dark ? "text-xs text-slate-500" : "text-xs text-[#8b857c]"}>—</span>
                      )}
                    </td>
                  </tr>
                ))}
