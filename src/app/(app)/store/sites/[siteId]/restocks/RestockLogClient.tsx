...
export default function RestockLogClient({
  role, canEdit, site, q,
  totalRestocks, totalQuantityAdded,
  restocks, exportRows, exportCols,
}: {
  role:     string;
  canEdit:  boolean;
  ...
  const printTitle   = `${site.name} Restock Log`;
    ...
    <div className={dark
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className={dark ? "mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-400" : "mt-3 max-w-2xl text-sm font-medium leading-6 text-[#857f76]"}>
                Review received stock history, suppliers, and restock notes for this site.
              </p>
              <div className={dark
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Link href={`/store/sites/${site.id}/restock`} className={dark
          ...
          <div className={dark
            ...
            <form className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <input name="q" defaultValue={q}
                placeholder="Search item, item code, supplier, received by..."
              ...
              {q && (
                <Link href={`/store/sites/${site.id}/restocks`} className={dark
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <SummaryCard dark={dark} label="Total Quantity Added" value={String(totalQuantityAdded)}
            sub="sum of visible restocks"
                ...
                {restocks.length === 0 ? (
                  ...
                  <tr key={row.id} className={dark ? "hover:bg-white/5" : "hover:bg-[#fcfaf7]"}>
                    ...
                    <td className={dark ? "px-5 py-3 text-slate-300" : "px-5 py-3 text-[#5d584f]"}>
                      {ITEM_TYPE_LABEL[row.inventoryItem.itemType] ?? row.inventoryItem.itemType}
                    </td>
                    <td className={dark ? "px-5 py-3 text-slate-300 text-center" : "px-5 py-3 text-[#5d584f] text-center"}>
                      {row.quantityAdded}{row.inventoryItem.unit ? ` ${row.inventoryItem.unit}` : ""}
                    </td>
                    <td className={dark ? "px-5 py-3 text-slate-300" : "px-5 py-3 text-[#5d584f]"}>
