  ...
  return (
    <div className={dark ? "min-h-screen bg-[linear-gradient(135deg,#0d1117_0%,#0f1923_50%,#0d1117_100%)] text-slate-200" : "min-h-screen bg-[linear-gradient(180deg,#fbf8f3_0%,#f5f2ed_48%,#f2ede5_100%)]"}>
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
        ...
        {/* RESTORED SITE DIRECTORY HEADER STYLE */}
        <section className={dark ? "relative overflow-hidden rounded-[28px] border border-white/10 bg-white/5 p-6 backdrop-blur-xl" : "relative overflow-hidden rounded-[28px] border border-[#e7ded3] bg-white/95 p-6 shadow-[0_16px_40px_rgba(26,24,20,0.06)]"}>
          ...
          {/* Stats Cards - Restored original look */}
        ...
        {/* RESTORED ORIGINAL TABLE DESIGN */}
            ...
            <table className="w-full table-auto text-[13px]">
              <thead className={dark ? "sticky top-0 z-20 bg-[#101720] text-left text-slate-400" : "sticky top-0 z-20 bg-[#f8f4ee] text-left text-[#5b564d]"}>
                <tr className="font-semibold">
                  ...
                  <th className="px-4 py-3 text-right">Condition</th> {/* Swapped Status for Condition */}
                  <th className="w-20 px-4 py-3 text-right no-print">Open</th>
                </tr>
              </thead>
              <tbody className={dark ? "divide-y divide-white/8" : "divide-y divide-[#eee7dd]"}>
                {filteredItems.map((item: any, index: number) => (
                  <tr 
                    key={item.id} 
                    className={dark ? "hover:bg-white/5 group" : "hover:bg-[#fcfaf7] group"}
                    onClick={() => router.push(`/store/sites/${site.id}/items/${item.id}/devices`)}
                  >
                    <td className="px-4 py-3" onClick={(e) => toggleSelect(item.id, e)}>
                    ...
                    <td className="px-4 py-3">
                      <div className={dark ? "font-semibold text-slate-100" : "font-semibold text-[#1a1814]"}>{item.name}</div>
                    ...
                    <td className="px-4 py-3 text-right no-print">
                      <div className="flex justify-end items-center gap-2">
                         <Link 
                            ...
                            className={dark ? "opacity-0 group-hover:opacity-100 text-[11px] font-bold text-sky-400 hover:underline" : "opacity-0 group-hover:opacity-100 text-[11px] font-bold text-blue-700 hover:underline"}
                            onClick={(e) => e.stopPropagation()}
                          >
                            EDIT
                          </Link>
                          <div className={dark ? "rounded-md border border-white/10 px-2 py-1 text-[11px] font-semibold text-slate-100" : "rounded-md border border-[#e0dbd2] px-2 py-1 text-[11px] font-semibold text-[#1a1814]"}>
                            OPEN
                          </div>
                      </div>
                    </td>
