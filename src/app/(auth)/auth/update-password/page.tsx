...
export default function UpdatePasswordPage() {
  ...
  useEffect(() => {
    // This catches the session from the URL automatically
    supabase.auth.onAuthStateChange((event, session) => {
      if (session) console.log("Session captured successfully");
    });
  }, []);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (updateError) {
      setError(updateError.message === "Auth session missing!" 
        ? "Verification failed. Please try clicking the email link again." 
        : updateError.message);
      setLoading(false);
    } else {
    ...
    <div className={dark ? "min-h-screen bg-[#0d1117] flex items-center justify-center px-4" : "min-h-screen bg-[#f5f2ed] flex items-center justify-center px-4"}>
      <div className={dark ? "w-full max-w-md rounded-[28px] border border-white/10 bg-white/5 p-8 backdrop-blur-xl shadow-2xl" : "w-full max-w-md rounded-[28px] border border-[#e7ded3] bg-white p-8 shadow-xl"}>
        <div className="flex flex-col items-center">
          <div className="grid h-16 w-16 place-items-center rounded-2xl bg-[#1d5fa8] text-white shadow-lg mb-6"><Lock size={28} /></div>
          <h1 className={dark ? "text-2xl font-bold text-slate-100" : "text-2xl font-bold text-[#1a1814]"}>Activate Account</h1>
          <p className="mt-2 text-center text-sm text-slate-400">Set your password to finalize registration.</p>
        </div>
        <form onSubmit={handleUpdate} className="mt-8 space-y-5">
          ...
          <button type="submit" disabled={loading} className="w-full rounded-xl bg-[#1d5fa8] py-4 font-bold text-white shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">
